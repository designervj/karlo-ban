...
export default function ProductGallery({ images }: { images: string[] }) {
          ...
          {images.map((src, i) => (
            <SwiperSlide key={i}>
              <img
                src={src}
                alt={`product-${i}`}
                className="md:h-[500px] h-[300px] w-full object-cover transition-transform duration-500 hover:scale-[1.02]"
              />
            </SwiperSlide>
          ))}
        </Swiper>
  ...
  breakpoints={{
    0: {
      slidesPerView: 3, // 👈 Mobile (up to 768px)
      spaceBetween: 8,
    },
    768: {
      slidesPerView: 6, // 👈 Tablet and above
      spaceBetween: 10,
    },
  }}
>
